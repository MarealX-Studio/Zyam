...
import { enhanceCodeHighlight, enhanceTableStyles } from '@/lib/markdown/enhanced-renderer'
...
export function MdEditor() {
  ...
  // Draw.io 集成
  const { isEditorOpen, openEditor, closeEditor, saveChart } = useDrawio()
  
  // 思维导图集成
  const {
    isEditorOpen: isMindmapOpen,
    openEditor: openMindmapEditor,
    closeEditor: closeMindmapEditor,
    saveMindmap
  } = useMindmap()
    ...
    const vditor = new Vditor('aritcle-md-editor', {
      ...
      preview: {
        ...
        parse: (element: HTMLElement) => {
          // 增强代码高亮
          enhanceCodeHighlight(element)
          // 增强表格样式
          enhanceTableStyles(element)
          ...
          blockquotes.forEach((blockquote) => {
            (blockquote as HTMLElement).style.cssText = `
              border-left: 4px solid var(--vditor-primary-color, #6366f1);
              padding: 1rem 1.5rem;
              margin: 1rem 0;
              background: var(--vditor-blockquote-bg, #f8fafc);
  ...
  // 处理 Draw.io 图表保存
  const handleDrawioSave = async (svgData: string, title?: string) => {
    try {
      const imageUrl = await saveChart(svgData, title)
      if (editor && typeof editor.insertValue === 'function') {
        editor.insertValue(`![${title || '图表'}](${imageUrl})`)
        if (typeof editor.focus === 'function') {
          editor.focus()
        }
      }
    } catch (error) {
      console.error('保存 Draw.io 图表失败:', error)
    }
  }

  // 处理思维导图保存
  const handleMindmapSave = async (svgData: string, title?: string) => {
    try {
      const imageUrl = await saveMindmap(svgData, title)
      if (editor && typeof editor.insertValue === 'function') {
        editor.insertValue(`![${title || '思维导图'}](${imageUrl})`)
        if (typeof editor.focus === 'function') {
          editor.focus()
        }
      }
  ...
  useEffect(() => {
    ...
    // 添加 Mermaid 和数学公式插入的事件处理
    const handleInsertMermaid = (event: any) => {
      const template = event as string
      if (editor && typeof editor.insertValue === 'function') {
        editor.insertValue(template)
        if (typeof editor.focus === 'function') {
          editor.focus()
        }
      } else if (editor && typeof editor.setValue === 'function' && typeof editor.getValue === 'function') {
    ...
    const handleInsertMath = (event: any) => {
      const template = event as string
      if (editor && typeof editor.insertValue === 'function') {
        editor.insertValue(template)
        if (typeof editor.focus === 'function') {
    ...
    emitter.on('toolbar-insert-mermaid', handleInsertMermaid)
    emitter.on('toolbar-insert-math', handleInsertMath)
    emitter.on('toolbar-drawio', handleDrawio)
    emitter.on('toolbar-mindmap', handleMindmap)
    
    return () => {
      emitter.off('toolbar-reset-selected-text')
      emitter.off('toolbar-insert-mermaid', handleInsertMermaid)
      emitter.off('toolbar-insert-math', handleInsertMath)
      emitter.off('toolbar-drawio', handleDrawio)
      emitter.off('toolbar-mindmap', handleMindmap)
    }
  }, [editor])
